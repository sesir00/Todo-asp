...
const TodoList = () => {
    ...
    const fetchTodos = async () => {
        ...
        try {
            ...
            if (err.response) {
                ...
                errorMessage += `Status: ${err.response.status}. Data: ${JSON.stringify(
                    err.response.data
                )}`;
            } else if (err.request) {
    ...
    const handleAddTodo = async (newTodoData) => {
        // Convert dueDate to ISO string here:
        if (newTodoData.dueDate) {
            newTodoData.dueDate = new Date(newTodoData.dueDate).toISOString();
        }
        console.log("Sending todo to API:", newTodoData);
        // Add optimistic UI update placeholder if desired
        setError(null); // Clear previous errors
        console.log("Trying to send this todo data:", newTodoData);
        try {
            ...
            {loading ? (
                ...
                <ul className="todo-list">
                    {todos.length === 0 && !loading ? (
                        <li className="no-todos-message" style={{ color: "black" }}>
